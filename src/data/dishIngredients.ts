...
export const dishRecipes: DishRecipe[] = [
  ...
  {
    // 2026-09-17 新菜（weeklyMenu id 37），hidden 待命。老板原话：食材与马铃薯炖花肉片
    // 相同，只把梅花肉片换成和牛片 → 份量逐项沿用上面那道（和牛片 125g 同梅花肉片）。
    // 和牛片是新采购 SKU，与「澳洲和牛饼」(按块) 不合并。
    name: '马铃薯炖和牛片',
    shortName: '和牛片',
    ingredients: [
      { name: '和牛片', qty: 125, unit: 'g' },
      { name: '马铃薯', qty: 100, unit: 'g' },
      { name: '白饭', qty: 80, unit: 'g' },
      { name: '黄洋葱', qty: 80, unit: 'g' },
      { name: '葱', qty: 2, unit: 'g' },
  ...
  },
  {
    ...
    // 2026-09-21 老板确认配菜：西兰花 50g（与家乡白萝卜焖花肉同口径），碗里没有别的。
    name: '豆酱焖排骨',
    shortName: '排骨',
    ingredients: [
  ...
  {
    // 新菜 2026-09-04 入系统（老板 09-04 定名+定价），09-10 周四上线主打。
    // 份量：老板 09-04 拍板「照搬同款」——与 #21 柠香香煎三文鱼饭完全同一套
    // （三文鱼 120g + 那四样配菜），两道只差酱汁。所以这里不是估的，是同款平移。
    // 蜜糖/酱汁按瓶装常备调味惯例不计入采购汇总（与柠檬、照烧酱同口径）。
    name: '蜜糖香煎三文鱼饭',
    shortName: '蜜鱼',
    ingredients: [
      { name: '三文鱼', qty: 120, unit: 'g' },
  ...
  {
    // 新菜 2026-09-18 入系统（老板定名，一切沿用 #32 蜜糖三文鱼：三文鱼 120g + 白饭 80g）。
    // 老板 09-18 拍板配菜只有西兰花：西兰花克数沿用 #32 的 50g 作基线，
    // TODO_CONFIRM 只剩一样配菜时碗妈会不会加量（毛豆/玉米/番茄已去掉）。
    name: '柚香香煎三文鱼饭',
    shortName: '柚鱼',
    ingredients: [
      { name: '三文鱼', qty: 120, unit: 'g' },
  ...
  {
    // 新菜 2026-09-26 入系统。份量老板 09-26 拍板「同豆酱焖排骨」（#28）：
    // 排骨 180g + 西兰花 50g + 白饭 80g。豆豉按惯例（瓶装常备调味）不计入采购汇总。
    name: '家乡豆豉焖排骨',
    shortName: '豉骨',
    ingredients: [
      { name: '排骨', qty: 180, unit: 'g' },
  ...
  {
    // 新菜 2026-09-26 入系统。碗里：鸡 + 马铃薯，无配菜无蛋（老板 09-26 确认）。
    // 马铃薯 100g 老板 09-26 给。
    // TODO_CONFIRM 鸡还没给：用哪个部位（鸡腿肉/鸡胸/全腿）、几 g 或几件 —— 别照抄别的菜。
    name: '家常土豆焖鸡',
    shortName: '薯鸡',
    ingredients: [
      { name: '马铃薯', qty: 100, unit: 'g' },
  ...
  {
    // 新菜 2026-09-04 入系统（老板 09-04 定名+定价），09-11 周五上线主打。
    // 份量：老板 09-04 拍板「照搬同款」——与 #14 香煎金黄鸡扒饭同一套（鸡扒 1 块
    // + 那三样配菜），两道只差酱汁与芝麻。
    // 白芝麻老板 09-04 明确「不列，当调味料」——用量小、不按份采购，与蜜糖/柠檬/
    // 酱油同口径。要改成按份采购的话，在这里加一行并给克数即可，盘点表会自动跟上
    // （ALL_RECIPE_INGREDIENTS 从本表派生，见 ingredientCatalog.ts）。
    name: '芝麻蜜汁鸡扒饭',
    shortName: '芝扒',
    ingredients: [
      { name: '鸡扒', qty: 1, unit: '块' },
...
export const addOnRecipes: Record<string, IngredientLine[]> = {
  ...
  // 食材行 = 各组成加料单点配方逐项相加（COMBO_COMPONENTS 不变式，dogfood 会验）。
  // A 档「下饭套」：蒜蓉西兰花炒蛋 + 荷包蛋 + 加饭
  '酱香下饭套 (原价 RM 15.40)': [
    { name: '西兰花', qty: 200, unit: 'g' },
    { name: '鸡蛋(生)', qty: 4, unit: '颗' }, // 炒蛋 3 颗 + 荷包蛋 1 颗
    { name: '白饭', qty: 70, unit: 'g' }, // 加饭生重 70g（150g 熟重的生米量）
  ...
  // wagyu-rice-king-combo：和牛饼自带温泉蛋 → 荷包蛋换马铃薯煎蛋（同姜葱下饭套）
  '和牛下饭套 (原价 RM 16.90)': [
    { name: '西兰花', qty: 200, unit: 'g' },
    { name: '马铃薯', qty: 150, unit: 'g' },
    { name: '鸡蛋(生)', qty: 5, unit: '颗' }, // 西兰花炒蛋 3 颗 + 马铃薯煎蛋 2 颗
  ...
  // C 档：马铃薯煎蛋 + 荷包蛋 + 加饭（原价 8.50 → 7.00）
  '排骨干饭套 (原价 RM 8.50)': [
    { name: '马铃薯', qty: 150, unit: 'g' },
    { name: '鸡蛋(生)', qty: 3, unit: '颗' }, // 马铃薯煎蛋 2 颗 + 荷包蛋 1 颗
    { name: '白饭', qty: 70, unit: 'g' }, // 加饭生重 70g（150g 熟重的生米量）
  ],
  '萝卜干饭套 (原价 RM 8.50)': [
    { name: '马铃薯', qty: 150, unit: 'g' },
  ...
  // ─── 2026-09-22 本周菜单全部补齐两档套餐（同上，组成加料逐项相加）──────
  // A 档「下饭套」：蒜蓉西兰花炒蛋 + 荷包蛋 + 加饭
  ...Object.fromEntries([
    '葱油下饭套 (原价 RM 15.40)',
    '蚝油下饭套 (原价 RM 15.40)',
    ...
    '山药下饭套 (原价 RM 15.40)',
    '柠香鸡胸下饭套 (原价 RM 15.40)',
  ].map((label): [string, IngredientLine[]] => [label, [
    { name: '西兰花', qty: 200, unit: 'g' },
    { name: '鸡蛋(生)', qty: 4, unit: '颗' }, // 炒蛋 3 颗 + 荷包蛋 1 颗
    { name: '白饭', qty: 70, unit: 'g' }, // 加饭生重 70g（150g 熟重的生米量）
  ]])),
...
export const addOnShortNames: Record<string, string> = {
  ...
  // 2026-09-21 补齐两档套餐
  '酱香下饭套 (原价 RM 15.40)': '酱香套',
  '酱油鸡干饭套 (原价 RM 7.00)': '酱鸡套',
  '和牛下饭套 (原价 RM 16.90)': '和牛套',
  '和牛干饭套 (原价 RM 7.50)': '牛饭套',
  '排骨下饭套 (原价 RM 15.40)': '排骨套',
  '排骨干饭套 (原价 RM 8.50)': '排饭套',
  '萝卜下饭套 (原价 RM 15.40)': '萝卜套',
  '萝卜干饭套 (原价 RM 8.50)': '萝饭套',
  '三文鱼下饭套 (原价 RM 15.40)': '三文套',
  '三文鱼干饭套 (原价 RM 8.50)': '三饭套',
  '鸡扒干饭套 (原价 RM 8.50)': '鸡饭套',
  // 2026-09-22 本周菜单全部补齐两档套餐
  '葱油下饭套 (原价 RM 15.40)': '葱油套',
  '葱油鸡干饭套 (原价 RM 8.50)': '葱饭套',
  '蚝油下饭套 (原价 RM 15.40)': '蚝油套',
  '蚝油鱼片干饭套 (原价 RM 7.00)': '蚝饭套',
  '土豆焖肉下饭套 (原价 RM 15.40)': '土豆套',
  '土豆焖肉干饭套 (原价 RM 7.00)': '土饭套',
  '和牛片下饭套 (原价 RM 15.40)': '牛片套',
  '和牛片干饭套 (原价 RM 7.00)': '片饭套',
  '参峇下饭套 (原价 RM 15.40)': '参峇套',
  '参峇干饭套 (原价 RM 7.00)': '参饭套',
  '山药下饭套 (原价 RM 15.40)': '山药套',
  '柠香鸡胸下饭套 (原价 RM 15.40)': '鸡胸套',
};
...
const MANUAL_LABEL_ALIASES: Record<string, string> = {
  ...
  // 2026-09-21 补齐两档套餐
  '酱香下饭套': '酱香下饭套 (原价 RM 15.40)',
  '酱油鸡干饭套': '酱油鸡干饭套 (原价 RM 7.00)',
  '和牛下饭套': '和牛下饭套 (原价 RM 16.90)',
  '和牛干饭套': '和牛干饭套 (原价 RM 7.50)',
  '排骨下饭套': '排骨下饭套 (原价 RM 15.40)',
  '排骨干饭套': '排骨干饭套 (原价 RM 8.50)',
  '萝卜下饭套': '萝卜下饭套 (原价 RM 15.40)',
  '萝卜干饭套': '萝卜干饭套 (原价 RM 8.50)',
  '三文鱼下饭套': '三文鱼下饭套 (原价 RM 15.40)',
  '三文鱼干饭套': '三文鱼干饭套 (原价 RM 8.50)',
  '鸡扒干饭套': '鸡扒干饭套 (原价 RM 8.50)',
  // 2026-09-22 本周菜单全部补齐两档套餐
  '葱油下饭套': '葱油下饭套 (原价 RM 15.40)',
  '葱油鸡干饭套': '葱油鸡干饭套 (原价 RM 8.50)',
  '蚝油下饭套': '蚝油下饭套 (原价 RM 15.40)',
  '蚝油鱼片干饭套': '蚝油鱼片干饭套 (原价 RM 7.00)',
  '土豆焖肉下饭套': '土豆焖肉下饭套 (原价 RM 15.40)',
  '土豆焖肉干饭套': '土豆焖肉干饭套 (原价 RM 7.00)',
  '和牛片下饭套': '和牛片下饭套 (原价 RM 15.40)',
  '和牛片干饭套': '和牛片干饭套 (原价 RM 7.00)',
  '参峇下饭套': '参峇下饭套 (原价 RM 15.40)',
  '参峇干饭套': '参峇干饭套 (原价 RM 7.00)',
  '山药下饭套': '山药下饭套 (原价 RM 15.40)',
  '柠香鸡胸下饭套': '柠香鸡胸下饭套 (原价 RM 15.40)',
...
export const COMBO_COMPONENTS: Record<string, string[]> = {
  ...
  '酱香下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],     // 2026-09-21
  '排骨下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],     // 2026-09-21
  '萝卜下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],     // 2026-09-21
  '三文鱼下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],   // 2026-09-21
  '和牛下饭套 (原价 RM 16.90)': ['蒜蓉西兰花炒蛋', '马铃薯煎蛋', '加饭'], // 2026-09-21
  ...
  '酱油鸡干饭套 (原价 RM 7.00)': ['荷包蛋', '加饭', '毛豆仁 (25g)'],   // 2026-09-21
  '和牛干饭套 (原价 RM 7.50)': ['温泉蛋', '加饭', '毛豆仁 (25g)'],     // 2026-09-21
  // C 档（2026-09-21）：马铃薯煎蛋 + 荷包蛋 + 加饭
  '排骨干饭套 (原价 RM 8.50)': ['马铃薯煎蛋', '荷包蛋', '加饭'],
  '萝卜干饭套 (原价 RM 8.50)': ['马铃薯煎蛋', '荷包蛋', '加饭'],
  '三文鱼干饭套 (原价 RM 8.50)': ['马铃薯煎蛋', '荷包蛋', '加饭'],
  '鸡扒干饭套 (原价 RM 8.50)': ['马铃薯煎蛋', '荷包蛋', '加饭'],
  // 2026-09-22 本周菜单全部补齐两档套餐
  '葱油下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '蚝油下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '土豆焖肉下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '和牛片下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '参峇下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '山药下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '柠香鸡胸下饭套 (原价 RM 15.40)': ['蒜蓉西兰花炒蛋', '荷包蛋', '加饭'],
  '蚝油鱼片干饭套 (原价 RM 7.00)': ['荷包蛋', '加饭', '毛豆仁 (25g)'],
  '土豆焖肉干饭套 (原价 RM 7.00)': ['荷包蛋', '加饭', '毛豆仁 (25g)'],
  '和牛片干饭套 (原价 RM 7.00)': ['荷包蛋', '加饭', '毛豆仁 (25g)'],
  '参峇干饭套 (原价 RM 7.00)': ['荷包蛋', '加饭', '毛豆仁 (25g)'],
  '葱油鸡干饭套 (原价 RM 8.50)': ['马铃薯煎蛋', '荷包蛋', '加饭'],
  // 主料加倍型
