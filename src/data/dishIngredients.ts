...
export const dishRecipes: DishRecipe[] = [
  ...
  },
  {
    ...
    //   白萝卜  基准 80g 熟 ↔ 113g 生（率 ×1.4125，老板 08-07 给）
    //           目标 90g 熟 → 90 × 1.4125 = 127.1 → 取 127g 生
    //           ⚠️ 这个率同时是加料「加白萝卜 (90g)」的口径（老板 08-07 拍板
    //           统一用它，取代 08-05 那版 90熟→120生 的 ×1.333）—— 两处都是
    //           90g 熟 / 127g 生，改一处必须同步改另一处。
    // 与其他菜一致：白饭存 80g **生米**采购重（非熟饭重）。
    name: '家乡白萝卜焖花肉',
    shortName: '萝卜',
    ingredients: [
      { name: '顶级有皮五花肉', qty: 156, unit: 'g' },
      { name: '白萝卜', qty: 127, unit: 'g' },
      { name: '西兰花', qty: 50, unit: 'g' },
      { name: '白饭', qty: 80, unit: 'g' },
    ],
  },
...
export const addOnRecipes: Record<string, IngredientLine[]> = {
  ...
  // 标签上的 90g 是**焖后熟重**。
  // 2026-08-07 老板拍板：熟→生统一走 ×1.4125（基准 80g熟↔113g生），
  //   90 × 1.4125 = 127.1 → 采购按 127g 生萝卜记。
  //   取代 08-05 那版 120g（当时用的是 ×1.333，与主菜口径差 6%）。
  // ⚠️ 主菜 30 号自带的白萝卜同样是 90g 熟 / 127g 生 —— 两处同一口径，
