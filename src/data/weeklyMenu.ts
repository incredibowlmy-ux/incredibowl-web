...
//
//   生效周：2026-08-24（Mon 24 Aug）。老板 08-19 定周一/周二，周三选 B。
//   ⚠️ 分两波推（今天周三晚 23:xx 是第一波）：
//     · 第一波（现在）：周二、周三、周一整块改完 —— 这三天的卡片指向 8/24–8/26
//       下周日期且当时 0 单，改动对本周零影响。周一先只放酱油鸡全腿。
//     · 第二波（周五 8/21 06:01 截单后）：当归蒸鸡全腿 #2 从周五挪到周一当主打。
//       现在不能挪 —— 本周五 8/21 已售 11 份且周四整天还能加单，提前动等于把
//       主打从网站上撕掉（同一道菜不能同时排周一和周五）。
//   本周变动：
//     · 周一：卤三层肉豆腐蛋 + 甜酸洋葱猪扒 → 转暂别；酱油鸡全腿(#1)回归
//       （07-27 暂别至今）。蒸鸡全腿周五截单后补进来当主打。
//     · 周二：三文鱼(#21 从周三挪来·主打) + 白萝卜焖花肉(#30 回归，08-17 暂别一周)。
//       金黄鸡扒(#14)让位挪到周三。
//     · 周三：豆酱焖花肉(#23·主打) + 金黄鸡扒(#14 从周二挪来)。
//     · 周四、周五：本波完全不动。
//   ⚠️ 待办（周五 06:01 第二波）：#2 挪到周一后，周五只剩常驻+鳗鱼 featured，
//      没有当天主打 —— 老板需指定周五 8/28 补哪道（候选：甜酸猪扒 #27 / 卤三层肉 #31）。
//
//   「一菜两天」仍只有照烧鳗鱼 (周二/五)：同一道菜不能在 WEEKLY_SCHEDULE
//   出现两次，故走 DAILY_DISHES + availableWeekdays + featureOnAvailableDays。
//   ⚠️ 代价：featured 菜拿不到 isPrimary，故周二 Hero 主打是三文鱼、周五是蒸鸡全腿。
// ═══════════════════════════════════════════════════════════════════
const WEEKLY_SCHEDULE: Record<number, number[]> = {
    1: [1],      // 周一：酱油鸡全腿(回归·暂代主打)；周五 06:01 后补当归蒸鸡全腿(#2)当主打
    2: [21, 30], // 周二：柠香三文鱼(主打·从周三挪来)、白萝卜焖花肉(回归)；照烧鳗鱼在常驻(featured)
    3: [23, 14], // 周三：家乡豆酱焖花肉(主打)、金黄鸡扒(从周二挪来)
    4: [20, 4],  // 周四：姜葱鱼片(主打)、绍兴酒蒸花肉 —— 本波不动
    5: [2],      // 周五：当归蒸鸡全腿(主打) —— 本波不动，8/21 已售 11 份
};

// 纳豆月见(全周)、马铃薯炖花肉片(全周)、古早味照烧鳗鱼饭(限周二/五·featured)
// —— 鳗鱼的供应日由 availableWeekdays 限定（菜单灰显 + 服务端拒收），
// 并在各供应日的每日精选列里展示。
...
const PAUSED_DISHES: { id: number; day: string }[] = [
    ...
    { id: 31, day: 'Mon / 周一' },    // 古早味卤三层肉豆腐蛋 暂别 2026-08-24（首发周后歇，让位酱油鸡/蒸鸡全腿）
    { id: 27, day: 'Mon / 周一' },    // 家乡甜酸洋葱猪扒 暂别 2026-08-24
];
...
const DISH_CATALOG: DishData[] = [
    ...
    },
    {
        ...
        price: 24.90,
        // 2026-08-10 周收成周三单日；2026-08-17 起改「周二+周五」两天供应 →
        // 常驻 + availableWeekdays + featureOnAvailableDays（白萝卜同款机制）。
