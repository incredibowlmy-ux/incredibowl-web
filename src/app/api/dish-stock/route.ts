...
// CORS: the file:// dashboard (库存 tab) reads this too, so responses must
// carry Access-Control-Allow-Origin like the /api/admin/* routes.
...
export async function GET() {
  try {
    ...
    return corsify(NextResponse.json(stock, { headers: { 'Cache-Control': 'no-store' } }));
  } catch (err) {
    console.error('dish-stock GET error:', err);
    return corsify(NextResponse.json({}, { headers: { 'Cache-Control': 'no-store' } }));
  }
}
