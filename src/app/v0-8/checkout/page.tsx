...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        ...
        else router.push('/v0-8/login') // Must be logged in to checkout
    }, [router])
        ...
        return (
            <div className="min-h-screen bg-white font-sans text-[#2D3142] flex flex-col items-center justify-center p-8 space-y-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
                    <Link href="/v0-8/account" className="flex-1 py-5 bg-[#4F6D7A] text-white rounded-3xl font-black text-sm text-center shadow-xl shadow-[#4F6D7A]/20">Track Order</Link>
                    <Link href="/v0-8" className="flex-1 py-5 bg-[#F7F9F9] text-[#4F6D7A] rounded-3xl font-black text-sm text-center">Back to Home</Link>
                </div>
            </div>
        )
    }
