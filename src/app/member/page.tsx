...
import { ArrowLeft, Star, ShoppingBag, Wallet, Calendar, Clock, CheckCircle, ChefHat, Truck, XCircle, Sparkles, Share2, Copy, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function MemberPage() {
    ...
    // Referral code (first 6 chars of UID)
    ...
    const handleShareWhatsApp = () => {
        const text = `🍛 我在 Incredibowl 订了好吃的家味便当！用我的推荐码 ${referralCode} 注册，双方各获 50 积分！\n👉 https://incredibowl.my`;
            ...
            <div className="max-w-2xl mx-auto px-4 -mt-14 space-y-5 pb-12">
                ...
                <div className="grid grid-cols-4 gap-3">
                    ...
                    <div className="bg-white rounded-2xl p-4 text-center shadow-sm border border-gray-100">
                        ...
                        <p className="text-[9px] text-gray-400 font-bold uppercase">最爱菜品</p>
                    </div>
                    <div className="bg-white rounded-2xl p-4 text-center shadow-sm border border-gray-100">
                ...
                {/* Favorite Dish Badge */}
                {favDish && (
                    <div className="bg-gradient-to-r from-[#FFF3E0] to-[#FFECB3] rounded-2xl p-4 flex items-center gap-3 border border-[#FFE0B2]">
                        <span className="text-3xl">👩‍🍳</span>
                        <div>
                            <p className="text-[10px] font-bold text-[#E65100] uppercase tracking-wider">你最常点的菜</p>
                            <p className="font-black text-[#1A2D23]">{favDish[0]} <span className="text-sm text-gray-400">({favDish[1]}次)</span></p>
                        </div>
                    </div>
                )}
                                        ...
                                        <div key={order.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            {isExpanded && (
                                                <div className="px-4 pb-4 animate-in slide-in-from-top-2 duration-200">
                                                    <div className="bg-[#F5F3EF] rounded-xl p-3 space-y-1 mb-3">
                                                    ...
                                                    {order.note && (
                                                        <p className="text-[10px] text-[#FF6B35] font-bold mt-1">📝 {order.note}</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
