...
export default function FaqHeroStrip({ locale = 'zh' }: { locale?: Locale }) {
    ...
    return (
        <button
            ...
            className="lg:hidden mt-2 group flex items-center gap-3 w-full text-left px-4 md:px-5 py-3 bg-white/70 hover:bg-white border border-[#FF6B35]/20 hover:border-[#FF6B35]/40 rounded-full shadow-sm transition-colors"
        ...
        >
            ...
            <span className="flex-1 min-w-0 flex flex-wrap items-baseline gap-x-2 text-[13px] md:text-[14px] leading-snug">
                <span className="font-extrabold text-[#1A2D23]">{t.question}</span>
                {/* Mobile: condensed copy fits one line. Desktop: full sentence. */}
                <span className="text-[#1A2D23]/75 sm:hidden">
                    {t.mobileBefore}<span className="font-semibold text-[#1A2D23]">Pearl Suria</span>{t.mobileAfter}
                </span>
                <span className="text-[#1A2D23]/75 hidden sm:inline">
                    {t.desktopBefore}<span className="font-semibold text-[#1A2D23]">Pearl Suria Residence</span>{t.desktopAfter}
                </span>
            </span>
