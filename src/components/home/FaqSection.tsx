    ...
    zh: [
        {
            ...
            a: (
                <>
                    没有店面哦 —— 碗妈是<span className="font-bold text-ink">家庭式私厨，只接外送</span>。<br />
                    我们从 Pearl Point 隔壁的 <span className="font-semibold text-ink">Pearl Suria Residence</span> 家里，煮好之后送到你家。
                </>
            ),
        },
        {
            ...
            a: (
                <>
                    在 Pearl Point 隔壁的 <span className="font-semibold text-ink">Pearl Suria Residence</span> 家里厨房。<br />
        ...
        },
        {
            ...
            a: (
                <>
                    主要送 <span className="font-semibold text-ink">{COVERAGE_AREAS.join(' / ')}</span> 一带：
                    ...
                    <p className="mt-3">
                        不确定家里在不在范围内？<span className="font-bold text-ink">注册账号 + 填写地址</span>，
        ...
        },
        {
            ...
            a: (
                <>
                    碗妈做的是<span className="font-bold text-ink">健康取向的中式家常菜</span>：全程<span className="font-bold text-ink">不加味精</span>，
                    每天凌晨去巴刹挑当天的鱼、肉和蔬菜，当天现煮现送。<br />
                    菜单上每道菜都标注<span className="font-bold text-ink">蛋白质克数</span>（不少主菜高蛋白 30g+），方便你按需求搭配。
                </>
            ),
        },
        {
            ...
            a: (
                <>
                    是的 —— 每天早上 <span className="font-bold text-primary">06:00 截单</span>（06:00 前下单当日配送）。<br />
    ...
    en: [
        {
            ...
            a: (
                <>
                    No shopfront &mdash; BowlMama is a <span className="font-bold text-ink">home-based private kitchen, delivery only</span>.<br />
                    We cook from a home in <span className="font-semibold text-ink">Pearl Suria Residence</span> (next to Pearl Point) and bring it to your door.
                </>
            ),
        },
        {
            ...
            a: (
                <>
                    A home kitchen in <span className="font-semibold text-ink">Pearl Suria Residence</span> (next to Pearl Point).<br />
        ...
        },
        {
            ...
            a: (
                <>
                    Mainly <span className="font-semibold text-ink">{COVERAGE_AREAS.join(' / ')}</span>:
                    <ul className="mt-2 space-y-1 text-[14px] md:text-[15px] lg:text-[15px]">
                        {DELIVERY_TIER_COPY.map((t) => (
                    ...
                    <p className="mt-3">
                        Not sure if you&apos;re in range? <span className="font-bold text-ink">Register an account and add your address</span> &mdash;
        ...
        },
        {
            ...
            a: (
                <>
                    BowlMama cooks <span className="font-bold text-ink">healthy-leaning Chinese home food</span>: <span className="font-bold text-ink">no MSG</span>, ever &mdash;
                    ingredients are picked fresh at the wet market every morning and cooked the same day.<br />
                    Every dish on the menu lists its <span className="font-bold text-ink">protein grams</span> (many mains are 30g+ protein), so you can plan your meals.
                </>
            ),
        },
        {
            ...
            a: (
                <>
                    Yes &mdash; daily cutoff is <span className="font-bold text-primary">06:00</span> (order before 06:00 for same-day delivery).<br />
    ...
    return (
        ...
        >
            ...
            <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
                {/* Header */}
                <div className="bg-line px-6 md:px-10 lg:px-14 py-6 lg:py-8 flex items-center gap-3">
                    <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-white/60 flex items-center justify-center shrink-0">
                        <HelpCircle size={20} className="text-ink lg:hidden" strokeWidth={2.5} />
                        <HelpCircle size={24} className="text-ink hidden lg:block" strokeWidth={2.5} />
                    </div>
                    <div>
                        <h2
                            id="faq-heading"
                            className="text-[22px] md:text-[28px] lg:text-[40px] font-extrabold tracking-tight text-ink leading-tight"
                        >
                            {t.heading}
                        </h2>
                        <p className="text-[13px] lg:text-base text-ink/65 font-medium leading-relaxed mt-0.5 lg:mt-1">
                            {t.sub}
                        </p>
                    </div>
                </div>
                        ...
                        return (
                            <li key={i}>
                                <button
                                    ...
                                    className="w-full flex items-center justify-between gap-4 text-left px-6 md:px-10 lg:px-14 py-5 lg:py-6 hover:bg-paper transition-colors active:bg-line/40"
                                >
                                    <span className="flex items-start gap-3 flex-1 min-w-0">
                                        <span className="shrink-0 inline-flex items-center justify-center w-7 h-7 lg:w-8 lg:h-8 rounded-full bg-primary/12 text-[#C84518] font-black text-[13px] lg:text-sm">
                                            Q
                                        </span>
                                        <span className="text-[15px] md:text-[17px] lg:text-[17px] font-extrabold text-ink leading-snug">
                                            {item.q}
                                        </span>
                                    </span>
                                    <ChevronDown
                                        size={20}
                                        strokeWidth={2.5}
                                        className={`shrink-0 text-ink/50 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
                                    />
                                </button>
                                {isOpen && (
                                    <div
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="text-[14px] md:text-[16px] lg:text-[17px] leading-[1.75] text-ink/85 font-medium">
                                                {item.a}
                                            </div>
                                        </div>
                                    </div>
