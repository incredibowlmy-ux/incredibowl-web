...
// 预填文案在 dict.ts 的 whatsAppStickyBar.prefilled。
// Trailing 🥡 is a silent source tag — Carmen can tell at-a-glance this came from the sticky bar.

export default function WhatsAppStickyBar({ locale = 'zh' }: { locale?: Locale }) {
    const t = HOME_DICT[locale].whatsAppStickyBar;
            ...
            <div className="pointer-events-auto max-w-md mx-auto lg:mx-0 lg:max-w-none bg-[#1A2D23] text-white rounded-2xl shadow-2xl shadow-black/40 backdrop-blur flex items-center gap-2.5 pl-3 pr-2 py-2 ring-1 ring-white/5">
                ...
                <span className="min-w-0 flex-1">
                    <span className="block text-[13px] font-bold leading-tight truncate">
                        {/* zh 是「首单立减 <RM 5>」、en 是「<RM 5> off your first order」——金额前后各留一个
                            槽位，空字符串在 React 里不产生文本节点，两边渲染结果与原文逐字一致。 */}
                        {claimed ? t.claimed : <>{t.firstOrderBefore}<span className="font-black text-[#FF9B50]">RM {FIRST_ORDER_PROMO_RM}</span>{t.firstOrderAfter}</>}
                    </span>
                    <span className="block text-[11px] text-white/60 leading-tight truncate">
                        {claimed ? t.claimedSub(FIRST_ORDER_PROMO_CODE) : t.claimSub}
                    </span>
                </span>
                {claimed ? (
                    <button
