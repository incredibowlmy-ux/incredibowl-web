...
export default function SubscribeModalEN() {
    ...
    useEffect(() => {
        ...
        // Page-hide as an exit signal — DESKTOP ONLY. On a phone it fires on
        // every app-switch (tapping WhatsApp, opening the gallery to fetch the
        // DuitNow receipt), so the customer came back to a popup mid-checkout.
        // Mirrors SubscribeModal.tsx.
        const onVisibility = () => {
    ...
    // Claim = remember the code locally; the cart auto-fills and (when signed in)
    // auto-applies it. Mirrors SubscribeModal.tsx.
    const claim = () => {
        ...
        >
            ...
            <div className="relative w-full max-w-sm bg-[#FDFBF7] rounded-[28px] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-6 sm:slide-in-from-bottom-2 zoom-in-95 duration-300">
                ...
                <div className="p-6">
                    ...
                    >
                        RM 5 off your first order — claim it right here
                    </h3>
                    <p className="text-sm text-[#1A2D23]/70 leading-relaxed mb-4">
                        One tap to claim; we apply it automatically at checkout — no waiting for a reply.
                        Pearl Point / OUG / Citizen 1 &amp; 2 neighbours are already eating.
                    </p>

                    {/* On-site self-serve claim — primary CTA. Previously the RM 5 lived
                        only in the WhatsApp script, so a new customer had to leave the
                        site and wait for a human to send a code. Mirrors SubscribeModal.tsx. */}
                    {claimed ? (
                        <button
                            type="button"
                            onClick={goOrder}
                        ...
                        >
                            <span className="flex items-center gap-1.5"><Check size={17} strokeWidth={3} /> Claimed — go pick a dish</span>
                            <span className="text-[11px] font-bold text-white/80">Code {FIRST_ORDER_PROMO_CODE} · applied at checkout</span>
                        </button>
                    ) : (
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <span>Claim RM {FIRST_ORDER_PROMO_RM} off</span>
                        </button>
                    )}

                    <p className="text-[11px] text-center text-[#1A2D23]/40 font-bold my-3">or</p>
                    ...
                    >
                        ...
                        <span>Add WhatsApp for new-dish alerts</span>
                    </a>
