...
import { X, Mail, Sparkles, Eye, EyeOff, LogOut, User as UserIcon } from 'lucide-react';
import Image from 'next/image';
import { signInWithGoogle, loginWithEmail, registerWithEmail, logout, onAuthChange } from '@/lib/auth';
...
export default function AuthModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
    ...
    // When modal opens, check if user is logged in
    useEffect(() => {
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-sm bg-[#FDFBF7] rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
                {/* Header */}
                ...
                {view === 'profile' && currentUser && (
                    <div className="p-8 space-y-6">
                        <div className="text-center space-y-3">
                            <div className="w-20 h-20 mx-auto rounded-full bg-[#E3EADA] flex items-center justify-center overflow-hidden border-2 border-[#E3EADA]">
                                {currentUser.photoURL ? (
                                    <img src={currentUser.photoURL} alt="Avatar" className="w-full h-full object-cover" />
                                ) : (
                                    <UserIcon size={32} className="text-[#1A2D23]" />
                                )}
                            </div>
                            <h3 className="font-bold text-[#1A2D23] text-xl">
                                {currentUser.displayName || '会员'}
                            </h3>
                            <p className="text-sm text-gray-500">{currentUser.email}</p>
                            <div className="inline-flex items-center gap-1 px-3 py-1 bg-[#E3EADA]/50 rounded-full text-xs font-bold text-[#1A2D23]">
                                <Sparkles size={12} className="text-[#FF6B35]" /> Incredibowl 会员
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="w-full py-3.5 bg-gray-100 text-gray-600 rounded-xl flex items-center justify-center gap-2 font-bold hover:bg-gray-200 transition-all disabled:opacity-50"
                        >
                            <LogOut size={18} />
                            {loading ? '登出中...' : '登出'}
                        </button>
                ...
                {view === 'main' && (
                    <div className="p-8 space-y-5">
                        ...
                        <div className="space-y-3">
                            ...
                            <button
                                disabled={true}
                                className="w-full py-3.5 bg-black text-white rounded-xl flex items-center justify-center gap-3 font-bold opacity-40 cursor-not-allowed"
                            >
                            ...
                            <button
                                disabled={true}
                                className="w-full py-3.5 bg-[#1877F2] text-white rounded-xl flex items-center justify-center gap-3 font-bold opacity-40 cursor-not-allowed"
                            >
