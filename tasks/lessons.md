...
## 2026-08-09 — push 前置检查不能和 push 串在同一条命令里

**现象：** `git log origin/main..main --oneline; git push` 一条命令连跑——检查输出打出来的
瞬间 push 已经在跑了，等于没检查。这次顺带把并行 session 留在本地的 2 个未推 commit
（白萝卜加料 + docs）一起发布了。恰好是周日凌晨低峰、内容也无害，纯属运气好；
07-24 就有过同型事故（并行 push 连带第 2 段菜单提前上线）。

**给自己的规则：**
1. `git log origin/main..main` 必须**单独一步跑完、亲眼读过输出**，确认每个 commit
   都是本次任务的，才发第二条命令 push。发现夹带别人的 commit → 停下来评估
   （时段、内容、是否 deferred），不确定就只报告不 push。
2. 本 repo 常态有多个并行 session 在 commit，「我刚 commit 的就是全部未推内容」
   这个假设永远不成立。

## 2026-08-04 — 读代码断言「某处漏了 X」之前，必须把那个函数/处理器**读到底**；验证没跑完不给结论

**现象：** 查 dashboard 删除订单按钮时，我读到
`await deleteDoc(...)` + `manual-voucher-release` 那几行就下了结论：「Dashboard 删除
订单不回补 dishStock/ingredientStock」，并据此建议老板「另开一轮修」。老板回「do it too」。
实际上再往下 30 行就是 `callAdminAPI('/api/admin/consume-stock', { release:true })`，
带 STOCK_ERA 闸门，**2026-07-02 就上线了**。是并行调研 agent 独立纠正了我。

**为什么危险：** 如果按「库存没回补」的前提再加一次回补，同一单会被 **+2 份 = 超卖**
（答应了做不出来的菜），比少卖严重得多。老板是基于我给的假信息批准动手的。
同一轮我还引入了一个真回归：孤儿 part 闸门把餐券和预付 credit 的折扣**加总**判断，
却只检查餐券的 claim 字段 → 只用了 credit 且自己带着 `addonCreditsUsed` 的 part
被误拦，pending 单还会卡住不放库存。

**根因：** ① 在一个长事件处理器里，只读到"我想找的那个 bug"就停止阅读，把
「我没看到回补」当成「它没有回补」。② 在验证（workflow / dry-run）还没跑完时就
输出结论并推动作。

**给自己的规则：**
1. 断言「某路径漏了 X」之前，**把那个函数/事件处理器从头读到 `}` 为止**，或
   `grep` 该处理器全域找 X 的关键词（本次只要 grep 一次 `consume-stock` 就不会错）。
   「我没看到」≠「它没有」。
2. 漏账方向要分清：**多退/多补是超卖方向，比少退严重**。提议任何"补一次回补"之前，
   先确认现在到底补了几次。
3. 闸门/判据涉及**多个独立资源池**（餐券、预付 credit、promo 券…）时，逐池分别判，
   绝不把金额加总再配一个池的 claim 字段。
4. 老板 2026-08-04 明确要求：**always wait for verification, only decide next step**。
   验证在跑就只汇报事实与进度，不给「要不要先做 X」这种把老板往动作上推的选项。
...
## 2026-08-09 · Bash 工具里绝不能用 PowerShell here-string

**症状**：`git commit -m @'...'@` 在 Bash 工具里跑完「成功」了，但 subject
变成了单独一个 `@`，正文全挤在 body 里。得 amend 重来。

**根因**：`@'...'@` 是 **PowerShell** 的 literal here-string 语法。Bash 工具跑的是
Git Bash（POSIX sh），`@` 对它只是普通字符，于是整段（含两个 `@`）被原样当成
提交信息。这台机器上两个 shell 工具并存，语法**不通用**。

**规则**：
1. Bash 工具里的多行字符串**一律用 heredoc**：`git commit -F -  <<'EOF'`，
   或写进临时文件再 `git commit -F <file>`（最稳，编码/换行都不会被吃）。
2. `@'...'@` 只在 **PowerShell 工具**里用，且结束的 `'@` 必须顶格。
3. 提交后**必须 `git log --oneline -1` 看一眼 subject**——commit 退出码 0
   不代表信息是对的，这次就是「命令成功、结果错」。

**更普遍的**：同一台机器上有多个 shell 工具时，别把上一个工具的语法惯性带过来；
写多行内容前先想「这行字是谁在解析」。

**🔁 2026-08-10 复发一次**（同样 `git commit -m @'...'@`，同样头尾多个 `@`，同样得 amend）。
写下规则不等于会执行——把它变成**手上的默认动作**：Bash 工具里要写提交信息，
手指直接打 `git commit -F - <<'MSG'`，**根本不去碰 `-m`**。看到自己敲了 `-m` 加
多行内容，就是走错路了，停下改成 heredoc。

**🔁 2026-08-11 又复发一次**（第三次，一字不差）。结论：靠「记住规则」已经证明
无效三次，改成**机器挡**——本机装了 `.git/hooks/commit-msg`，subject 是 `@`、或
